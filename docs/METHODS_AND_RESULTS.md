...
The raw dataset is 8,075 rows spanning 295 distinct extractant molecules and 23 f-element elements (the 15 lanthanides plus the actinides Am, Cm, Cf, Np, Pu, Pa, Th, and U), present as 28 distinct oxidation-state species (for example Np in the III, IV, V, and VI states). It is split into two molecule-disjoint files, `Training_Data_V27.csv` for training and validation and `Testing_Data_V39.csv` as a held-out test set, so that no extractant molecule appears in both. Because the table is wide but shallow in distinct chemistry (only 295 molecules), interpolating new conditions for a molecule already seen is much easier than extrapolating to a genuinely new molecule. That asymmetry is why results are reported on two separate tracks rather than a single number.
...
A focused evaluation (`scripts/sep_factor_eval.py`, `scripts/sep_factor_confidence.py`) measures the separation factor, the difference in logD between two f-elements at the same extractant and conditions, across all 4,598 f-element pairs (23 elements, 28 oxidation-state species, including actinides), obtained by differencing the logD model (a direct delta model was tested in `sep_factor_model.py` and lost outright, signed R2 -1.92, so differencing is used). It is reported in two regimes and, per the project's main contribution, always with the confidence-filtered operating point. Pairs are matched on extractant, acid type and concentration, temperature, and extractant concentration; metal concentration may differ between the two endpoints (it does for about 58 percent of pairs) and is handled as a model feature rather than held fixed, so the differenced target is selectivity at matched solution conditions rather than at identical metal loading.
...
With our confidence layer (known extractant, most confident 10 percent): direction accuracy 0.789, Spearman 0.776, and the normalized error RMSE/spread falls to 0.64, which beats a random-ranking shrinkage benchmark of 0.80. This gain was verified by a committed script (`scripts/sep_factor_confidence_verify.py`, output `results/sep_factor_confidence_verify_results.csv`): within each true-gap quartile the most-confident half is more accurate than the least-confident half at the same gap size (gap-balanced direction advantage +0.116, mid-gap bins +0.20 to +0.24), and the most-confident tenth beats the full set on direction by +0.067 (paired bootstrap 95 percent CI [+0.029, +0.103], p=0.0002), so it is neither a gap-selection artifact nor variance shrinkage. For a new extractant confidence is not net-useful (direction declines to 0.586; the RMSE drop is pure shrinkage), because the per-row confidence is too weakly calibrated on unseen ligands (rho 0.20 versus 0.39). The confidence-filtered figures are the headline for this task; the full-coverage numbers are never reported alone.

Dr. Zhang's own model on the same task (`scripts/sep_factor_zhang.py`, his XGBoost on his ECFP-centric features, differenced) ties ours at full coverage on known extractants (known signed R2 0.369 versus our 0.356, direction 0.715 versus 0.726); on new extractants his model is marginally ahead on all three metrics (signed R2 0.227 versus 0.188, direction 0.672 versus 0.656, Spearman 0.507 versus 0.462), from the fingerprint our descriptor-only setup drops. Both hit the same wall at full coverage, so the ceiling is the data, not the model. Our contribution is the confidence layer, which is model-agnostic: applied to his predictions it lifts his most-confident-tenth known signed R2 from 0.369 to 0.470, and applied to ours from 0.356 to 0.591, with our model leading on the confident slice (direction 0.789 versus his 0.704). His published model does not itself include a confidence ranking. Figures `figures/sep_factor_eval.png`, `figures/sep_factor_confidence.png`, `figures/sep_factor_zhang.png`, and `figures/sep_zhang_confidence.png` (the confidence-forward view: Zhang as-published versus both models with our confidence layer, ours leading on the confident slice).
...
## AutoData candidate generator, selectivity, and the new-extractant confidence bake-off

AutoData (arxiv 2606.25996) is implemented as a candidate-EXTRACTANT generator, not a training-data augmenter (pseudo-labeling with the model's own predicted labels was already shown not to improve the model). An agentic loop runs four rounds of four parallel LLM generators that propose novel SMILES; the honest scorer (a bootstrap-bagged LightGBM on RDKit descriptors plus metal and condition features, differenced across two metals for the separation) gates chemical validity and dataset-novelty and scores the separation with an ensemble-disagreement uncertainty; a per-round evaluator analyzes the failures and rewrites the generation strategy. Real scores are written to results CSVs by the scorer, never taken from agent text. The first run (Am/Eu) produced 142 candidates at 95 percent parse-validity, 105 unique novel-versus-dataset molecules (novel means new to the 295-extractant dataset, not new to chemistry: the top hits recapitulate the known Cyanex-301 dithiophosphinic-acid family, which is a soundness check on the generator), across 56 Murcko scaffolds. The generator rediscovers the right soft-donor families but does not beat the best known extractants: novel predicted-separation p90 is about 0.99 (a separation factor near 10x) against the in-sample known p90 of about 1.32 (near 22x); the medians are about equal and within noise. The reflection review returned ship-with-fixes, and the in-sample-baseline wording, the parse-validity-versus-uniqueness split, and the uncertainty label were corrected.

The central honesty check is whether the model rewards genuine selectivity (separating two metals) or merely general extraction strength (extracting everything, which separates nothing). On known extractants the model captures real selectivity: predicted separation is uncorrelated with general strength (Spearman -0.20), the signed gap flips sign across extractants, and soft S-donors are correctly the most Am-selective (median signed gap 1.07 versus 0.49 for hard O/P-donors), with 150 extractants measured for both Am and Eu supporting it. On novel generated molecules the signal partly collapses into strength (Spearman +0.53, 99 percent favor Am), the extrapolation weakness. The ranking is therefore strength-adjusted (each candidate's own mean logD is subtracted before ranking), and generalized to every f-element pair: across 295 extractants 66 percent of the centered cross-metal variance is one common shape and 34 percent is extractant-specific, so pairwise selectivity is genuine but roughly one-third extractant-specific. With separation set as the explicit search target (each molecule scored by the best pair it separates), the pool is split into two confidence-gated shortlists, EXPLORE (best pair over at least 5 measured extractants) and TRUSTED (best pair over at least 20), each reported at the top-25 percent and top-10 percent confidence tiers.

Because the deployed learned-error confidence layer does not sharpen new-extractant separation (top-10 percent signed R2 falls from 0.19 to about 0.00 and direction from 0.66 to 0.59), a bake-off of six uncertainty estimators was run on the same molecule-grouped out-of-fold predictions and pairs, with the winner chosen by a pre-registered composite (average lift in the shrinkage-immune direction accuracy, Spearman, and useful-separation F1 at the top-25 and top-10 percent tiers, plus calibration). Bootstrap-bagged ensemble disagreement won: it is the only method that improves the new-extractant regime, lifting top-10 percent direction from 0.66 to 0.81, useful-F1 from 0.44 to 0.59, signed R2 from about 0.00 to 0.28, and calibration from 0.20 to 0.31 (both R2 and RMSE reported; the RMSE fall is partly variance shrinkage, which is why the shrinkage-immune metrics lead). The applicability-domain distances (ECFP Tanimoto novelty and descriptor k-nearest-neighbor distance) that were expected to win lost badly (calibration about 0.01 to 0.02), and the incumbent reproduces its prior 0.188 signed R2 / 0.656 direction exactly, a leakage tripwire. The winner is wired into the AutoData scorer, and the known-extractant confidence layer is unchanged because it already works (top-10 percent signed R2 0.36 to 0.59). Scripts: autodata_score.py, autodata_pool.py, autodata_bestpair.py, autodata_bp_pool.py, sep_selectivity_check.py, sep_allpairs.py, newext_confidence_bakeoff.py. Figures: autodata_pool.png, autodata_selectivity.png, autodata_allpairs.png, autodata_bestpair_confident.png, newext_confidence_bakeoff.png. Specs: specs/2026-07-01-autodata/ and specs/2026-07-01-newext-confidence/.
...
The numbers are reproducible on the cleaned set and are an upper bound relative to the raw measurements; the irreducible label-noise floor is about 0.445 log units. The committed `results/` and `figures/` are the reference outputs, so a re-run should match them within seeding noise. Two things will not reproduce out of the box: the Zhang scripts need his `/tmp` featurized files. The `.gitignore` merge-conflict block flagged in an earlier draft has since been resolved.
