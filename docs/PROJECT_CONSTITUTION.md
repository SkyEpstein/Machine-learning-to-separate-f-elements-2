...
11. **Always update everything.** Any change propagates in the same pass to every artifact it touches: the deck, the workbook and the master results table, the README, the methods and manuscript-figure docs, the figures, and the changelog. Never leave some artifacts stale or inconsistent.
12. **Spec-driven development for everything.** Every substantive change runs through the SDD lifecycle (the `spec-feature` skill): specify, clarify, plan, select by bake-off, analyze cross-artifact consistency, implement, verify with an adversarial reflection review, and ship. Scale the ceremony to the size, but never skip the ask-first, bake-off, verify, and update-everything steps.
