...
| Quantity | Model | Metric | RMSE | n | Evaluation | Caveat | Source |
|---|---|---|---|---|---|---|---|
...
| Separation factor WITH CONFIDENCE (our model, known, top 10%) | logD diff + err model | signed R2 0.591 | 0.373 | 459 | condition-key, top 10% by confidence | Direction 0.789, Spearman 0.776, normalized RMSE 0.64 (beats random-rank 0.80, verified). The confidence layer is our differentiator; for new extractants confidence is not net-useful | sep_factor_confidence_results.csv |
| Separation factor, Zhang XGBoost (known) | his XGBoost + ECFP, differenced | signed R2 0.369 | 1.281 | 4598 | condition-key CV, matched conditions | Ties our 0.356 (dir 0.715 vs 0.726); but his model has NO confidence layer, so it cannot reach our confidence-filtered direction 0.789 | sep_factor_zhang_results.csv |
