...
> Note: The two tracks measure different things. Track A is new-extractant screening (no molecule in both train and test); Track B is condition optimization for a molecule already seen, so its number is higher by construction. All scores are on the label-QC-cleaned data (replicate logD range ≤ 2), which makes them an upper bound relative to the raw measurements; the irreducible label-noise floor is about 0.45 log units. The most-confident-slice figures (Track A 0.912, Track B 0.940) are selective-prediction operating points at 10% coverage, not overall accuracy, and the Track B figure is being recomputed on the leakage-free condition-key split.
