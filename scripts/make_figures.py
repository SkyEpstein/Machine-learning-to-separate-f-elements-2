...
def finish(fig, name):
    ...
    fig.tight_layout(); fig.savefig(f"figures/{name}.png", bbox_inches='tight', facecolor='white'); pdf.savefig(fig); plt.close(fig)
    _im = Image.open(f"figures/{name}.png").convert('RGBA'); _bg = Image.new('RGB', _im.size, (255, 255, 255)); _bg.paste(_im, mask=_im.split()[3]); _bg.save(f"figures/{name}.png"); saved.append(name)
